...
def create_df_NH_noether(filenames_data , files_data):
    ...
    #rename columns
    df_cols = ['#Refine', 'deg', '#DoF', '#CG','Solve Time(s)','MDoFs/Sec', 'Strain Energy','Petsc Time(s)', 'Total Time(s)','np','run']
    ...
    df = df.sort_values(['deg', '#Refine', 'np', 'run'], ascending = (True, True,True,True))
...
def draw_paper_data(df):
    mdf = df.drop(['#CG','MDoFs/Sec','Petsc Time(s)', 'np'], axis=1)
    mdf['Solve Time(s)'] = np.round(mdf['Solve Time(s)'], decimals=2)
    mdf['Total Time(s)'] = np.round(mdf['Total Time(s)'], decimals=2)
...
def run_alg_perf():
    ...
    #     50_TubeRef_2_deg_4_cpu_64_run_3.log
    keep_idx = [2,4,8]
